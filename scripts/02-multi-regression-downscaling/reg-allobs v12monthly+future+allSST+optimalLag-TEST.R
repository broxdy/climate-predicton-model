...
dir_name="reg-allobsV12monthly cal1971-1985 vrf1986-2000 sim1971-2000-future+allSST+optimalLag-TEST"
...
#### add SST
data.newset = cbind(data.obs.sel,data.gcm.sel,data.sst0[1:row.n.obs,])
...
obs.all = data.obs[,-c(1:2)]
max.cor.lag = 6
cat("\nDetermine optimal lag for SST")
min.row.n = min(dim(obs.all)[1],dim(data.sst)[1])
...
n.all.method = array(NA, c(obs.n,method.n,2))
...
for (obs.i in 1:obs.n){
			for (gcm.i in 1:gcm.n){
						...
						### add SST
						for(sst.i in 1:sst.n){
						gcm0[,gcm.n-sst.n+sst.i] = c(rep(NA,opt.lag[obs.i,sst.i]),data.sst0[,sst.i])[1:row.n.obs]
						}
...
##Write correlation
setwd(dir1)
write.csv(obsccf, file= "Matrix-corr of all.csv", row.names = FALSE)
setwd(dir0)

##Write n.used
setwd(dir1)
write.csv(nccf, file = "Matrix-corr of all_n.csv", row.names = FALSE)
setwd(dir0)
...
start0.cal=1971  #year start in table file
startyear.cal=1971 #year to calibration begins
endyear.cal=1985 #year to calibration END  # cal 1971-1985
endday.cal=0


start0=start0.cal  #year start in table file
startyear=startyear.cal #year to calibration begins
endyear=endyear.cal #year to calibration END  # cal 1971-1985
endday=endday.cal
...
cat("\n *** Adding SST")
for(sst.i in 1:sst.n){
gcm0[,gcm.n-sst.n+sst.i] = c(rep(NA,opt.lag[obs.i,sst.i]),data.sst0[,sst.i])[1:dim(gcm0)[1]]
data[,2+obs.n+gcm.n-sst.n+sst.i] = c(rep(NA,opt.lag[obs.i,sst.i]),data.sst0[,sst.i])[1:dim(data)[1]]
colnames(gcm0)[gcm.n-sst.n+sst.i] = paste(colnames(data.sst0)[sst.i],"lag",opt.lag[obs.i,sst.i])
colnames(data)[2+obs.n+gcm.n-sst.n+sst.i] = paste(colnames(data.sst0)[sst.i],"lag",opt.lag[obs.i,sst.i])
}
...
for (i in 1:method.n){
	...
	par(mai =  c(.8, .8, .8, .8))
	cal.cbind = na.omit(cbind(obs.cal[,obs.i],as.real(calf)))
	#lmx = max(calf[not.na.row],max(obs.cal[not.na.row,obs.i]))
	#lmn = min(calf[not.na.row],min(obs.cal[not.na.row,obs.i]))
	lmx = max(cal.cbind)
	lmn = min(cal.cbind)
	matplot(cal.cbind[,1],cal.cbind[,2],pch="X",xlab="Observation",ylab="Prediction",xlim=c(lmn,lmx),ylim=c(lmn,lmx),type="p",col = "red", main=paste("Model calibration",station[obs.i],"method:",method[i]))
	...
	par(mgp=c(2.5,1,0), ps =16, mfrow=c(1,1))
	par(mai =  c(.8, .8, .8, .8))
	#not.na.row = as.real(rownames(as.data.frame(na.omit(cbind(obs.vrf[,obs.i],as.real(prd))))))
	vrf.cbind = na.omit(cbind(obs.vrf[,obs.i],as.real(prd)))
	lmx = max(vrf.cbind)
	lmn = min(vrf.cbind)
	matplot(vrf.cbind[,1],vrf.cbind[,2],pch="X",xlab="Observation",ylab="Prediction",xlim=c(lmn,lmx),ylim=c(lmn,lmx),type="p",col = "red", main=paste("Model verification",station[obs.i],"method:",method[i]))
	matlines (c(-1000,1000), c(-1000,1000), type = "l", lty = "dashed", lwd = 1, pch = NULL, col = 1:2)
...
best.caleq[obs.i] = min(which(calerror[,5,obs.i]==min(calerror[,5,obs.i],na.rm = TRUE)))
...
for(i in 1:obs.n){
print.table.prd = rbind(print.table.prd,cbind(paste("method",1:method.n),sel.prd.all[i,,],mthd.bic[i,],calerror[,3:7,i],prderror[,3:7,i]))
}
print.table.prd = cbind(print.table.prd[,1],print.table.prd[,-1])
colnames(print.table.prd) = c("method","station","number of prd",paste("pr",1:upper.max.nprd,sep="-"),"bic","cal-ME","cal-ABSE","cal-RMSE","cal-NS","cal-N","vrf-ME","vrf-ABSE","vrf-RMSE","vrf-NS","vrf-N")
...
startyear=2000 #year to calibration begins
endyear=2009 #year to calibration END  # cal 1971-1985
endday=8
ifinal = which(data.sst$year==endyear)[length(which(data.sst$year==endyear))] + endday
datafinal = length(data.sst$year)+1
subty = (startyear-start0)*12
...
gcm.future0 = cbind(gcm.future0,array(NA,c(dim(gcm.future)[1],sst.n)))
...
for(i in 1:obs.n){
		...
		# GCM cal
		gcm0[,gcm.n-sst.n+sst.i] = c(rep(NA,opt.lag[i,sst.i]),data.sst0[,sst.i])[1:dim(gcm0)[1]]
		data[,2+obs.n+gcm.n-sst.n+sst.i] = c(rep(NA,opt.lag[i,sst.i]),data.sst0[,sst.i])[1:dim(data)[1]]
		colnames(gcm0)[gcm.n-sst.n+sst.i] = paste(colnames(data.sst0)[sst.i],"lag",opt.lag[i,sst.i])
		colnames(data)[2+obs.n+gcm.n-sst.n+sst.i] = paste(colnames(data.sst0)[sst.i],"lag",opt.lag[i,sst.i])

		# GCM future
		if(opt.lag[i,sst.i]>0){
			sst.future = rbind(data.sst0[1:opt.lag[i,sst.i],],data.sst0)
			sst.future.i = sst.future[-c(1:dim(gcm0)[1]),sst.i]
			#sst.future.rown = dim(sst.future)[1]
			gcm.future0[,gcm.n-sst.n+sst.i] = c(sst.future.i,rep(NA,dim(gcm.future0)[1]-length(sst.future.i)))
			}else{
				sst.future.i = data.sst0[-c(1:dim(gcm0)[1]),sst.i]
				gcm.future0[,gcm.n-sst.n+sst.i] = c(sst.future.i,rep(NA,dim(gcm.future0)[1]-length(sst.future.i)))
				}
		colnames(gcm.future0)[gcm.n-sst.n+sst.i] = paste(colnames(data.sst0)[sst.i],"lag",opt.lag[i,sst.i])
		}
