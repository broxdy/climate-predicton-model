...
for(i in 1:3){
for(j in 1:nparmat){
plot(ts2[,i],main=paste(colnames(submt2)[i],"(Blue) - ",colnames(submt2)[j],"(Red)"),xaxt = "n",xlab="",ylab=colnames(submt2)[i],lty=1,col=4,lwd=0.5,xaxt = "n")
jan <- time(ts2)[cycle(ts2[,i]) == 1]  # january
mtick <- matrix(time(ts2),nrow=12)[c(1,4,7,10),] # 3 month
abline(v = jan, lty = 2)
...
par(new=TRUE)
plot(ts2[,j],main="",xlab="",ylab="",axes=FALSE,lty=1,col=2,lwd=0.5)
}
}
dev.off()
setwd(dir0)
