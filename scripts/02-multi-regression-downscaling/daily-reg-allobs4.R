...
filledpredictant = cbind(predictantdata,matrix(NA, nrow = length(predictantdata), ncol=(extprd+13)))
filledpredictant = data.frame(filledpredictant)
filledpredictant[,1] = NA
colnames(filledpredictant)= c("Predictant",colnames(data0[,colpool[firstprd:extprd]]),"First Predictor","Last Predictor","nPredictor","n-cal","n-used","n-mising","avrg error","RMSQ error","NS coef","cor","R2","AIC","Predictors")
...
# for 1 dimension predictor
		...
		# Mark used predictor #
		filledpredictant[cmp.row.predictor,(firstprd+1):(nprd+1)] = 1
		filledpredictant[cmp.row.predictor,(extprd+2)] = firstprd
		filledpredictant[cmp.row.predictor,(extprd+3)] = nprd
		filledpredictant[cmp.row.predictor,(extprd+4)] = nprd-firstprd+1
		filledpredictant[cmp.row.predictor,(extprd+5)] = length(fit$residuals)
		filledpredictant[cmp.row.predictor,(extprd+6)] = length(cmp.row.predictor)

		filledpredictant[cmp.row.predictor,(extprd+8)] = mean(fit$residuals)
		filledpredictant[cmp.row.predictor,(extprd+9)] = (mean((fit$residuals)^2))^0.5
		filledpredictant[cmp.row.predictor,(extprd+10)] = 1-sum((fit$residuals)^2)/sum((predictor_eq-mean(predictor_eq))^2)
		filledpredictant[cmp.row.predictor,(extprd+11)] = cor(fit$model[,1],fit$fitted.values)
		filledpredictant[cmp.row.predictor,(extprd+12)] = (cor(fit$model[,1],fit$fitted.values))^2
		filledpredictant[cmp.row.predictor,(extprd+13)] = AIC(fit)
colt = ""
for(np in firstprd:nprd){colt = paste(colt,np)}
		filledpredictant[cmp.row.predictor,(extprd+14)] = colt
filling = length(predictantdata[which(filledpredictant[,extprd+14]==colt),1])-length(removeNA(predictantdata[which(filledpredictant[,extprd+14]==colt),1]))
		filledpredictant[cmp.row.predictor,(extprd+7)] = filling
								}
						}
