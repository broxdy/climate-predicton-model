...
### Set ML Equation #
if((df-firstprd) == 0){fit <- lm(predictor_eq~mydata)
		}else{
		fit <- lm(predictor_eq~.,data=mydata)
		}
...
filledpredictant = cbind(predictantdata,matrix(0, nrow = length(predictantdata), ncol=extprd))
filledpredictant = data.frame(filledpredictant)
filledpredictant[,1] = NA
colnames(filledpredictant)= c("Predictant",colnames(data0[,colpool[firstprd:extprd]]))
...
if(length(cmp.row.predictor)!= 0){
	if(length(cmp.row.predictor)>=1){
		filledpredictant[cmp.row.predictor,1] = predict(fit,na.omit(frameprd))
...
cmp.row.predictor = rownames(na.omit(predictordata[which(is.na(filledpredictant[,1])==TRUE),]))
#which(is.na(predictordata[which(is.na(filledpredictant[,1])==TRUE),])==FALSE)
...
# for 1 dimension predictor
		if((df-nprd)==0){
					...
					filledpredictant[which(is.na(predictordata[which(is.na(filledpredictant[,1])==TRUE),])==FALSE),1] = fit$coefficients[2]*t+ fit$coefficients[1]}
		#filledpredictant[cmp.row.predictor,1] = predict(fit,cmp.data.predictor)
		# Mark used predictor #
		filledpredictant[cmp.row.predictor,(firstprd+1):(nprd+1)] = 1
								}
