...
dir_name="reg-allobsV12daily cal1971-1985 vrf1986-2000 sim1971-2000-future"
...
gcm.f.list = c("EXTRACTED ECHO-G_A1B_2046-2065.csv",
"EXTRACTED ECHO-G_A2_2046-2065.csv",
"EXTRACTED ECHO-G_B1_2046-2065.csv",
"EXTRACTED ECHO-G_A1B_2081-2100.csv",
"EXTRACTED ECHO-G_A2_2081-2100.csv",
"EXTRACTED ECHO-G_B1_2081-2100.csv")
future.date.col = c(1:4)

obs.f = "Daily Filled all climate 1971-2006_V2.csv"
gcm.f = "ECHO-G daily 1971-2000.csv"
...
data0.obs = data.obs[,-c(1:3)]
data0.gcm = data.gcm[,-1]
#data0.gcm = data.obs[,-c(1:3)]

obs.n = ncol(data0.obs)
gcm.n = ncol(data0.gcm)
...
for(year.i in 1:n.year.obs){
	gcm.date.factor = obs.freq[year.i]/gcm.freq
	n.day.list.gcm = c(n.day.list.gcm,year.i*1000+ c(1:gcm.freq)*gcm.date.factor)
}
...
if(runthisloop){
######## **** ##########
#REMOVE data like to be predicted
######## **** ##########
start.rmv.year = 1986
end.rmv.year = 2000
row.first.rmv = which(data.obs$Year == start.rmv.year)[1]
row.last.rmv = which(data.obs$Year == end.rmv.year)[length(which(data.obs$Year == end.rmv.year))]

data0rmv.obs = data0.obs[1:row.n.obs,]
data0rmv.obs[c(row.first.rmv:row.last.rmv),] = NA
}else{data0rmv.obs = data0.obs[1:row.n.obs,]}
...
####################
## Creat new  data set for prediction below this part
####################
cat("\nCreat new data set")
data.newset = data0rmv.obs
for(gcm.i in 1:gcm.n){
	f <- approxfun(n.day.list.gcm,data0.gcm[,gcm.i],yleft = data0.gcm[1,gcm.i])
	data.newset = cbind(data.newset,f(n.day.list.obs))
	colnames(data.newset)[obs.n+gcm.i] = colnames(data0.gcm)[gcm.i]
	}

data.newset = cbind(data.obs$Year[1:row.n.obs],calnd.day.list.obs,data.newset)
colnames(data.newset)[1:2] = c("year","day")
...
calerror = array(NA, c(method.n,7,obs.n))
prderror = array(NA, c(method.n,7,obs.n))
...
for (i in 1:method.n){
	...
	lmx = max(calf,max(obs.cal[,obs.i]))
	lmn = min(calf,min(obs.cal[,obs.i]))
	...
	calerror[i,3,obs.i] = mean(cal[which(is.na(cal[,1])==FALSE),2]-cal[which(is.na(cal[,1])==FALSE),1])
	calerror[i,4,obs.i] = mean(abs(cal[which(is.na(cal[,1])==FALSE),1]-cal[which(is.na(cal[,1])==FALSE),2]))
	calerror[i,5,obs.i] = sqrt(mean((cal[which(is.na(cal[,1])==FALSE),1]-cal[which(is.na(cal[,1])==FALSE),2])^2))
	calerror[i,6,obs.i] = 1-sum((cal[which(is.na(cal[,1])==FALSE),2]-cal[which(is.na(cal[,1])==FALSE),1])^2)/sum((cal[which(is.na(cal[,1])==FALSE),1]-mean(cal[which(is.na(cal[,1])==FALSE),1]))^2)
	...
	lmx = max(prd,max(obs.vrf[,obs.i]))
	lmn = min(prd,min(obs.vrf[,obs.i]))
...
setwd(dir0)
cat("Predictant,GCM,Mothed,MERROR,ABSERROR,RMSE,Nash–Sutcliffe,AIC\n") 
for(i in 1:obs.n){
	for(mthd in 1:method.n){
		cat(calerror[mthd,1,i],",",calerror[mthd,2,i],",",method[mthd],",",calerror[mthd,3,i],",",calerror[mthd,4,i],",",calerror[mthd,5,i],",",calerror[mthd,6,i],",",calerror[mthd,7,i],"\n")
			 }
		 }
sink()
...
for(i in 1:obs.n){
	for(mthd in 1:method.n){
		cat(prderror[mthd,1,i],",",prderror[mthd,2,i],",",method[mthd],",",prderror[mthd,3,i],",",prderror[mthd,4,i],",",prderror[mthd,5,i],",",prderror[mthd,6,i],",",prderror[mthd,7,i],"\n")
			 }
		 }
sink()
}#END if(run.vrf)
