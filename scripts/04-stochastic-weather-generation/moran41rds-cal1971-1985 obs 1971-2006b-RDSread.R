...
dir_name = "StochasticPCPv41rds_cal1971-1985 obs1971-2006bRDSread"
show.v.chart = FALSE
...
## Input for observation in verification
data.vrf.daily = "Daily Filled all climate 1971-2006+Wet_V2.csv"

## Input files (rainfall generation)
future.f.mean.rain = "PCP obs1971-2006.csv"
...
future.f.rainy.day = "WDY OBS1971-2006.csv"
...
############# verification files
## Daily data
data.daily.vrf =  read.table(data.vrf.daily, stringsAsFactors=FALSE,header=TRUE, sep=",",dec=".",na.strings="-")
daily.unusedcol.data = 19
daily.unusedcol2.data = 44
# also remove "X478201" (col. 21)
data.daily.vrf = data.daily.vrf[,-c(1:daily.unusedcol.data,21,(daily.unusedcol2.data+1):dim(data.daily.vrf)[2])]
...
### Set day to predicted result ts
#daily.day.list = org.f[,3]
#daily.month.list = org.f[,2]
#daily.year.list = org.f[,1]
#data.daily = data.daily.vrf
daily.day.list = org.f[1:10960,3]
daily.month.list = org.f[1:10960,2]
daily.year.list = org.f[1:10960,1]
data.daily = data.daily.vrf[1:10960,]
future.month.list.2000 = future.month.list[1:360]
...
setwd(dir1)
pdf(paste("daily-Conclude Rain Gen+Obs all sta all 2000.pdf"), width = 10 ,height = 6)
#par(mgp=c(2,0.8,0), ps =18)
par(mgp=c(2.5,0.8,0), ps =18)
par(mai = c(0.8, 0.8, 0.8, 0.3))
setwd(dir0)
...
for(i in 1:12){
			...
			for(j in 1:max(daily.day.list[row.sel])){
										...
										#if(max(rain.plot, na.rm=TRUE)>200){cat(i,",",j,",",k)}
										for(k in 1:weather.gen.predict.realiz.n){
																	lines(ts.show,as.real(unlist(rain.plot[,station.i,k])),col = "red",type = "p", lwd = 1, cex = 0.2)
																	}
										lines(ts.show,as.real(unlist(data.daily[row.sel[row.sel.2],station.i])),type="h", col = "blue")
...
future.month.list = future.month.list.2000
setwd(dir1)
pdf(paste("monthly-Conclude average-rlz Rain Gen+Obs all sta 2000.pdf"), width = 10 ,height = 6)
#par(mgp=c(2,0.8,0), ps =18)
par(mgp=c(2.5,0.8,0), ps =18)
par(mai = c(0.8, 0.8, 0.8, 0.3))
setwd(dir0)
