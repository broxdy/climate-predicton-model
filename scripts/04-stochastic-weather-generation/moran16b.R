...
dir_name = "predict_v16b_predict rainyday"
...
cat("\n## Month :",month.i,"Start year :",future.all.year[year.i]," ##")
#year.i = 1
...
# use SAME seed as in v.predict.gen.seed
#random.rain.prob = seed.u.gen.predict[1:day.in.month,1,1]
...
##
#cat("\nRainy by normal random ",length(rainy.n.pos1),":",rainy.n.pos1,"\nRainy by MCM generation\n")
...
#				limit.rainy.day.n = 1
#				choose.rainy.day = 1
#		  	  }else{limit.rainy.day.n = rainy.day.n[sta]}
...
re.seed.u.m.all = array(runif(weight.n*weather.gen.predict.realiz.n,0,1), c(weight.n,weather.gen.predict.realiz.n))
...
for(uni in 1:unique.n){
	cat("\n\n-- Generating for",unique.rainy.day.n[uni],"day(s):",station.title[sel.unique.station[1:sel.unique.station.n[uni],uni]])

	v.normal.gen = array(NA, c(weight.n,unique.rainy.day.n[uni],weather.gen.realiz.n)) # set the matrix
	...
	for(n in 1:unique.rainy.day.n[uni]){
		...
		for(seed.n in 1:weather.gen.predict.realiz.n){
									...
									# calculate rainfall amount at station n
									rain.predict.gen[month.i,,pos.unique.rain.day[n,uni],seed.n] = -log(1-v.normal.gen[,n,seed.n])*array(as.numeric(mean.rain),c(weight.n,1))
									} # END seed.n
		
		# Average the realization to rain.predict.gen.avg
		if(sel.unique.station.n[uni] == 1){rain.predict.gen.avg[month.i,sel.unique.station[1:sel.unique.station.n[uni],uni],pos.unique.rain.day[n,uni]] = mean(rain.predict.gen[month.i,sel.unique.station[1:sel.unique.station.n[uni],uni],pos.unique.rain.day[n,uni],],na.rm = TRUE)
						          }else{rain.predict.gen.avg[month.i,sel.unique.station[1:sel.unique.station.n[uni],uni],pos.unique.rain.day[n,uni]] = rowMeans(rain.predict.gen[month.i,sel.unique.station[1:sel.unique.station.n[uni],uni],pos.unique.rain.day[n,uni],],na.rm = TRUE)}
	...
	###
	# start (ELSE) when zero rain // if(rainy.day.n[uni] != 0)
...
for(i in 1:station.n){rain.predict.gen.avg[month.i,i,which(is.na(rain.predict.gen.avg[month.i,i,1:day.in.month]))] = 0}
