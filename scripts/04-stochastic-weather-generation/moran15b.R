...
dir_name = "predict_v15b_predict rainyday"
...
############
cat("\nRecord mcm.eq.table")
colnames(mcm.eq.table)[c(1:3,5,6)] = c("Statio.i","Station","month","abs.mean.error","rmsq.error")
...
prob.rainy = rainy.n*coef.2+coef.1
# Set the day postion which random prob. >  limit prob of rainy day
random.rain.prob = runif(day.in.month)
##################################################################
rainy.n.pos = which(random.rain.prob*100 > prob.rainy[1]) # still only station [1]
##################################################################

##
cat("\nRainy by normal random ",length(rainy.n.pos1),":",rainy.n.pos1,"\nRainy by MCM generation",length(rainy.n.pos),":",rainy.n.pos)
