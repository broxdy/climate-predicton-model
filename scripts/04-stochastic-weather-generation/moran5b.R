...
#### Generate V, rain.predict spattially autocorrelation random variables
				...
				for(n in 1:ma.coef.n){
							...
							# plot to pdf
							cat(" // Plot moving average coefficint :",ma.coef.range[n],"\n")
							
							#for(k in 1:seed.v.ndist.n){

											plot(rain.predict[,n], type = "l", xlab = paste("moving avrg coef. =",round(ma.coef.range[n],digits=2),"   at average",round(mean.rain,digits=1),"mm.(chart avg.=",round(mean(rain.predict[,n]),digits=1),"mm.)   of month:",rain.i), ylab = "Rainfall (mm)", lty = 1, lwd = 2)
											par(new = TRUE)
