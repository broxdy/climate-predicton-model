...
#weather.gen.predict.realiz.n = 300 # realization to predict to build curve daily I(rain)


maindir = "Temp/Stochastic"
dir_name = "predict_v18_indv macoef_monthlyMoran_30rlz+uniqGenModel"
...
for(uni in 1:unique.n){
	...
	# set cuumulative distribution model
		# check if NO rainy day
	if(pos.unique.rain.day[1,uni] == 0){v.normal.model.gen = ecdf(v.predict.gen[1,,])
							}else{
								day.pos = pos.unique.rain.day[which(!is.na(pos.unique.rain.day[,uni])),uni]
								v.normal.model.gen = ecdf(v.predict.gen[day.pos,,])
								}
...
for(i in 1:station.n){
				rain.predict.gen.avg[month.i,i,which(is.na(rain.predict.gen.avg[month.i,i,]))] = 0
				for(rlz.i in 1:weather.gen.predict.realiz.n){rain.predict.gen[month.i,i,which(is.na(rain.predict.gen[month.i,i,,rlz.i])),rlz.i] = 0	}
				}
...
for(rlz.i in 1:weather.gen.predict.realiz.n){rain.predict.conclude.allrlz[(start.data.date-start.org.date +1):(end.data.date-start.org.date + 1),,rlz.i] = matrix(rain.predict.gen[month.i,,1:day.in.month,rlz.i], ncol = station.n, nrow = day.in.month, byrow = TRUE)}



#print.rain.predict.conclude = rain.predict.conclude[-which(is.na(rain.predict.conclude[,1])),]
setwd(dir1)
write.csv(rain.predict.conclude, file = "conclude average rain.csv")
setwd(dir0)
...
###########
conclude.allrlz.rearng = daily.365day.list
col.n = 1
#rearrange rain.predict.conclude.allrlz
for(station.i in 1:station.n){
for(rlz.i in 1:weather.gen.predict.realiz.n){

								conclude.allrlz.rearng = cbind(conclude.allrlz.rearng,rain.predict.conclude.allrlz[1:total.n,station.i,rlz.i])
								col.n = col.n + 1
								colnames(conclude.allrlz.rearng)[col.n] = paste(station.title[station.i],rlz.i,sep = "-")
								}
								}
colnames(conclude.allrlz.rearng)[1] = "Days"
# print.rain.predict.conclude = rain.predict.conclude[-which(is.na(rain.predict.conclude[,1])),]
setwd(dir1)
write.csv(conclude.allrlz.rearng, file = "conclude rain all rlz.csv")
setwd(dir0)
