...
cat("Read file list\n")
sheetf = "GCMv4_real-mo future prd list-A1B.csv"
#sheetf = "test.csv"
sheet.list <- read.table(sheetf, stringsAsFactors=FALSE,header=TRUE, sep=",",dec=".",na.strings = "-")
# Output name
out.file = "output"
## specify output folder here
dir_name ="GCMv4-A1Bextracted"

L=file.exists(dir_name)  
if(!L) dir.create(dir_name)
dir0=dirname(file.path("D:","CMIP3","Future Scenario","GCMv4-A1B","dummy"))
dir1=dirname(file.path("D:","CMIP3","Future Scenario","GCMv4-A1B",paste(dir_name),"dummy"))
