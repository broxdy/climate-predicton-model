...
sheetf = "GISS-ER-20c3m-atm-da-onlyfile.txt"
...
dir0=dirname(file.path("C:","Downloads","CMIP3","GISS-ER","dummy"))
dir1=dirname(file.path("C:","Downloads","CMIP3","GISS-ER",paste(dir_name),"dummy"))
...
if(length(rownames(var.table)) < length(rownames(output.table)))	{
											if(extend.dat == TRUE)	{
															var.table = rbind(var.table.extend,var.table)
															extend.dat = FALSE
															}else	{
																extend.dat = TRUE
...
cat(paste("\nReaded:",datafile[i]))
cat(paste("\n",colnames(var.table)))
if(extend.dat == TRUE)	{cat(" - will be extended")}
