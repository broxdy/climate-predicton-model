...
cat("Read file list\n")
sheetf = "nerc_list.csv"
#sheetf = "test.csv"
sheet.list <- read.table(sheetf, stringsAsFactors=FALSE,header=TRUE, sep=",",dec=".",na.strings = "-")
# Output name
out.file = "output"
## specify output folder here
dir_name ="NERC_mo-extracted"

L=file.exists(dir_name)  
if(!L) dir.create(dir_name)
#dir0=dirname(file.path("D:","CMIP3","Future Scenario","GCMv4-B1","dummy"))
#dir1=dirname(file.path("D:","CMIP3","Future Scenario","GCMv4-B1",paste(dir_name),"dummy"))

dir0=dirname(file.path("D:","temp","ncdf","dummy"))
dir1=dirname(file.path("D:","temp","ncdf",paste(dir_name),"dummy"))


#@@@####################### END change parameter here #########################
...
out.id = cbind(sheet.list[,1],matrix(data = NA, nrow = length(sheet.list[,1]), ncol = 20,))
...
mync.time.start.year = 1870
#mync.time.calendarday = 365 # Day in a calendar year
mync.time.calendarday = 12 # 12 months time step
...
# search sub-id from NC file
subvar.srch.id = which(out.id[i,]=="plev")-2 # Initial for predictor
##** lev
sub.pr.name = "plev"
							if(length(which(out.id[i,]==sub.pr.name))>0){
							subvar.srch.id = which(out.id[i,]==sub.pr.name)-2
	  					   	}else{
##** height
sub.pr.name = "height"
							if(length(which(out.id[i,]==sub.pr.name))>0){
...
sub.pr.name = "depth"
							...
							subvar.srch.id = which(out.id[i,]==sub.pr.name)-2
								...
								cat("Not found sub-id")
								}
								}}}
# END search sub-id from NC file
...
# if have 3 dimension
if(var.info$ndims == 3)	{
		cat("Dimension:3 reading predictor\n")
		...
		cat("/Dimension:3\n")
					...
					# if have 4 dimension
					if(var.info$ndims == 4)	{
									cat("Dimension:4 reading predictor\n")
									...
									for(j in 1:subvar.n)	{
													...
													cat(paste("/Dimension:4",var.name,format(subvar.name[j],scientific = FALSE),"\n"))
...
# Show file name
cat(paste("Reading File:",datafile[i]))
...
if(i>1){
							if(extend.dat == TRUE)	{
											# change the column name to fit each other
											colnames(var.table.extend) = colnames(var.table)

											# combine rows into var.table
...
cat(paste("GRID:\n lat  =",var.get.nc(nc,mync.lat.id)[mync.grid.lat],"(",mync.grid.lat,")\n long =",var.get.nc(nc,mync.long.id)[mync.grid.long],"(",mync.grid.long,")\n")) 
if(largefile==FALSE)	{
