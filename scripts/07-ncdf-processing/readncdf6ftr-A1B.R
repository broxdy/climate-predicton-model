...
sheetf = "future file list A1Bv2.csv"
...
dir0=dirname(file.path("D:","CMIP3","Future Scenario","A1B","dummy"))
dir1=dirname(file.path("D:","CMIP3","Future Scenario","A1B",paste(dir_name),"dummy"))
...
if(length(which(out.id[60,]=="plev"))>0){subvar.srch.id = which(out.id[60,]=="height")-2
							}else{subvar.srch.id = which(out.id[i,]=="plev")-2}
...
setwd(dir1)
write.csv(table1, file=paste(out.file,run.list[1],".csv"))
if(length(run.list)>1){write.csv(table2, file=paste(out.file,run.list[2],".csv"))}
if(length(run.list)>2){write.csv(table3, file=paste(out.file,run.list[3],".csv"))}
if(length(run.list)>3){write.csv(table4, file=paste(out.file,run.list[4],".csv"))}
setwd(dir0)
