...
sheetf = "NCAR_pcm1-20c3m-atm-da1960+file_run.csv"
...
largefile = TRUE # when TRUE it will not engage ts() funtion and the origin date have to be yyyy/1/1
...
mync.time.id = 5 # Position for time dimension
mync.long.id = 3 # Position for longtitude
mync.lat.id = 4 # Position for lat
...
mync.time.start.year = 0000
mync.time.calendarday = 365 # Day in a calendar year

L=file.exists(dir_name)  
if(!L) dir.create(dir_name)
dir0=dirname(file.path("D:","CMIP3","NCAR-PCM1","dummy"))
dir1=dirname(file.path("D:","CMIP3","NCAR-PCM1",paste(dir_name),"dummy"))
