...
# prefix name for working directory
#dir_name = "ML-regrsn-PredictionV4_1971-1999"
o_dir_name = "ML-PredictV8"

# conclusion directory
#c_dir_name = "ML-regrsn-prediciton-conclusionV5_1971-1999"
o_c_dir_name = "ML-PredictV8 conclude"
...
dir_name = paste(o_dir_name,sub(".csv","",indexf.future),sep="__")
c_dir_name = paste(o_c_dir_name,sub(".csv","",indexf.future),sep="__")

# ratio to put into calibration part
#verifyfactor = 0.5
#verifyfactor = 0.821 # to seperate cal/vrf at 1971-1999/2000-200
...
maxpredictor = config.nprd[,2] # maximum number of predictors in extra prediction (extprd)
...
endyear.obs=2100 # if 12/2000 have to set to endyear= 2000
...
dataindex = rbind(dataindex, dataindex.future)
...
repeat{
	...
	if(length(ignore.colpool)>0){	cat("\n !! Skip:", colnames(data.index)[colpool[ignore.colpool]])
...
##
## end of SUB3b to change colpool to away N/A column####
...
## under here for calibration and verification 13 column
		filledpredictant[cmp.row.predictor,(extprd+16)] = endcal
		filledpredictant[cmp.row.predictor,(extprd+17)] = endvrf-endcal
		...
		filledpredictant[cmp.row.predictor,(extprd+20)] = 1-sum((fitc$residuals)^2)/sum((predictor_eq[1:endcal]-mean(predictor_eq[1:endcal]))^2)
		...
		filledpredictant[cmp.row.predictor,(extprd+26)] = 1-sum((fitv-obsv)^2)/sum((predictor_eq[(endcal+1):endvrf]-mean(predictor_eq[(endcal+1):endvrf]))^2)
...
if(gloop == lcol){
setwd(dir2)
write.csv(all.prd.calvrf,file="Calibration+Verification.csv")
setwd(dir0)
...
pivot.ns[jj,ii] = mean(pivot.all[k1:k2,ns.col[ii]])
