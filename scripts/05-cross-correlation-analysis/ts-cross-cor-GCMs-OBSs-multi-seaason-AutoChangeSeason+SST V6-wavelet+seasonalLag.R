...
#sst.f = "ocean index 1971-2009test.csv"
...
#prefix.dir = "cross_cor-GCM_OBSs-V6+optimal-lagSSTs"

optimal.lag = FALSE 
#optimal.lag = TRUE # Run SSTs at optimal lag

#plot.pdf.seasonal.lag = FALSE
plot.pdf.seasonal.lag = TRUE

print.cross.wavelet = FALSE
#print.cross.wavelet = TRUE # **!!PRINT!!** only at directory of i.season=1 !!++!!
...
if(i.season == 2){print.cross.wavelet = FALSE}
	...
	for (i in 1:n.index){
		...
		for (j in 1:n.obs){
					...
					if(length(which(is.na(newmat[,j])))==0){
												...
												if(length(which(is.na(newmat[,c(j,i+n.obs)])))>0){
																				t.na = max(which(is.na(newmat[,i+n.obs])))
																				t.start = time(newmat)[t.na+1]
																				newmat2 = ts(newmat[(t.na+1):nrow(newmat),], frequency=12,start=t.start)
																				n.wcs <- wcs(newmat2[,i+n.obs],newmat2[,j],s0=2,noctave=3,nvoice=30,labsc=1.5,plottitle=paste("\n",short.tscolname[i+n.obs],"  -  ",short.tscolname[j]))
							...
							for(i.month in 1:12){
								if(define.season[i.month, i.season] == 1){ season1 = rbind(season1,window(eq.trend,c(startyear,i.month),deltat=1))
									...
									if(plot.this.pdf){
										...
										#Plot all data set
										plot(lag(newmat[,i+n.obs],h),newmat[,j], cex = 0.2,type = "p",col="white",main=paste(colnames(newmat)[i+n.obs]," (Lag= ",h,")",sep=""), ylab=short.tscolname[j],xlab=colnames(newmat)[i+n.obs])
