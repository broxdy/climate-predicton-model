...
dir_name="ts-autocrt08_seasonal-all4-4season-nocharts-crs-wvlt"
	...
	for (i in 1:n.index){
		...
		for (j in 1:n.obs){
					if(length(which(is.na(newmat[,j])))==0){
												#n.wsp <- wsp(newmat[,j],s0=2,noctave=3,nvoice=20,nreal=0,labsc=1,plottitle=paste("Index:",colnames(newmat)[i+n.obs]," \Met.:",colnames(newmat)[i+n.obs]))
