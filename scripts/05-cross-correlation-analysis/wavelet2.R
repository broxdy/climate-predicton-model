...
wsp <- function(ts,s0=1,noctave=5,nvoice=10,w0=2*pi,sw=0,tw=0,swabs=0,siglevel=0.95,critval=NULL,nreal=1000,arealsiglevel=0.9,kernel=0,markt=-999,marks=-999,logscale=FALSE,plot=TRUE,units="",device="screen",file="wsp",color=TRUE,pwidth=10,pheight=7,labsc=1,labtext="",plottitle="",sigplot=3){
  ...
  if (class(ts)!="ts"){
    ...
    #PLOTTING
    if (plot)
      plot(wclist,markt,marks,NULL,NULL,logscale,FALSE,units,plottitle,
      device,file,FALSE,color,pwidth,pheight,labsc,labtext,sigplot)
...
wcs <- function(ts1,ts2,s0=1,noctave=5,nvoice=10,w0=2*pi,sw=0,tw=0,swabs=0,markt=-999,marks=-999,logscale=FALSE,phase=TRUE,plot=TRUE,units="",plottitle="",device="screen",file="wcsp",split=FALSE,color=TRUE,pwidth=10,pheight=7,labsc=1,labtext=""){
  ...
  if (class(ts1)!="ts"){
    ...
    if (plot) plot(wclist,markt,marks,NULL,NULL,logscale,phase,units,paste("Wavelet Cross Spectrum: ",plottitle),device,file,split,color,pwidth,pheight,labsc,labtext)
...
plotwt <-
  function(wt,phs,t,s0,noctave,w0,cv=NULL,at=NULL,markt=-999,marks=-999,t1=NULL,t2=NULL,logscale=FALSE,phase=FALSE,units="",plottitle="",device="screen",file="wavelet",split=FALSE,color=TRUE,pwidth=10,pheight=4,labsc=1,labtext="",sigplot=1,xax=NULL,xlab=NULL,yax=NULL,ylab=NULL){
...
#  if (phase==TRUE && split==FALSE) nlo <- layout(matrix(c(1,2,3,4),2,2,byrow=TRUE),widths=c(4,1))
 #  else nlo <- layout(matrix(c(1,2),ncol=2,byrow=TRUE),widths=c(5,1))
   else nlo <- layout(matrix(c(1,2,3,4),ncol=2,byrow=TRUE),widths=c(9,1,9,1))
  par(mai=c(1.0, 0.8, 0.5, 0.0))
  ...
  if (logscale){
    if (units=="")
      ...
      image(x=t,z=log10(wt),col = colors,axes=FALSE,xlab=paste("Time ","[",units,"]",sep=""),ylab=paste("Scale ","[",units,"]",sep=""),frame.plot=TRUE,cex.lab=labsc)
  }
  else{
    if (units=="")
      image(x=t,z=wt,col = colors,axes=FALSE,xlab="Time",ylab="Scale",frame.plot=TRUE,cex.lab=labsc,mgp=mgpv)
    else
      image(x=t,z=wt,col = colors,axes=FALSE,xlab=paste("Time ","[",units,"]",sep=""),ylab=paste("Scale ","[",units,"]",sep=""),frame.plot=TRUE,cex.lab=labsc)
  }
  ...
  image(z=rangebar,axes=FALSE,col=colors,frame.plot=TRUE,cex.lab=labsc)
