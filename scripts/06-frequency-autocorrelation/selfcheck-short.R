...
dataindex <- read.table("index1950.csv", stringsAsFactors=FALSE,header=TRUE, sep=",",dec=".",na.strings="-")
...
datafile = c("index1950.csv","obs1950.csv")
...
for(loop in 1:1){
data <- read.table(datafile[1], stringsAsFactors=FALSE,header=TRUE, sep=",",dec=".",na.strings="-")
...
start0=1950  #year start in table file
startyear=1950 #year to analysis begins
startmonth=1
startmonth0=1
endyear=2007
endmonth=0
...
# for short print
setwd(dir1)
pdf(paste("short-selfcheck.pdf"))
acf0=acf(newmat2, drop.lag.0 = FALSE, ylim = c(-1,1),lag.max = 12)
setwd(dir0)
