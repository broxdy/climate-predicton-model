...
dataindex0 =dataindex[-c(0:subty,(ifinal+1):datafinal),c(3:4,7,51:63)]
...
setwd(dir1)
sink(paste("Rain-Correlation.csv"))
cat("P1,P2,")
for(m in 0:24){cat(-1+m/12,",")}
cat("\n")

pdf(paste("Rain-mutlti-corl.pdf"), width= 5, height = 30)
par(mfcol=c(13,1))

for(i in 3:3){
for(k in 4:16){
ccf0=ccf(dataindexts[,i],dataindexts[,k], ylim = c(-1,1),lag.max = 12, plot = FALSE, na.action = na.exclude)
plot(ccf0, main = paste(colnames(dataindexts)[i]," - ",colnames(dataindexts)[k]),ylab ="Cross-Correlation",xlab= "Lag (month)",ylim = c(-0.5,0.5))
