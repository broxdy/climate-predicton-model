...
f.list = "file-listSDSMselSTA.txt"
...
f.title.mid =c("A1Bt1","A1Bt2","A2t1","A2t2","B1t1","B1t2")
f.title.suffix = ".out"
...
f.title.full1 = paste(f.title.prefix,f.title, sep = "")
f.title.full = integer(0)
for(i in 1:length(f.title.mid)){f.title.full = c(f.title.full,paste(f.title.full1,"_",f.title.mid[i],f.title.suffix, sep = ""))}
f.title = sub(".out","",f.title.full)
